...
/**
 * Server-side proxy for test-studio mutations.
 *
...
const ROUTES: Array<{ pattern: RegExp; upstream: (match: RegExpExecArray) => string }> = [
  { pattern: /^cases$/, upstream: () => '/test-suites' },
  { pattern: /^runs$/, upstream: () => '/test-runs' },
  {
    pattern: new RegExp(`^runs/(${UUID})/sync$`),
    upstream: (match) => `/test-runs/${match[1]}/sync`,
  },
];
...
export async function POST(request: Request, context: { params: Promise<{ action?: string[] }> }) {
  ...
  if (!upstreamPath) {
    return Response.json({ message: 'Unknown test operation' }, { status: 404 });
  }
  ...
  try {
    const upstream = await fetch(`${baseUrl}${upstreamPath}`, {
      method: 'POST',
      cache: 'no-store',
      // Starting a run creates any missing provider test mappings and dispatches the
      // run in one call, so it can take noticeably longer than a simple mutation.
