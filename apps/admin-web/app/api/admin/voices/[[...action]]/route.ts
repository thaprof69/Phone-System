...
/**
 * Server-side proxy for voice mutations.
 *
...
const ROUTES: Array<{ pattern: RegExp; upstream: (match: RegExpExecArray) => string }> = [
  { pattern: /^refresh$/, upstream: () => '/voices/refresh' },
  {
    pattern: new RegExp(`^(${UUID})/approve$`),
    upstream: (match) => `/voices/${match[1]}/approve`,
  },
  { pattern: /^assignments$/, upstream: () => '/voice-assignments' },
];
...
export async function POST(request: Request, context: { params: Promise<{ action?: string[] }> }) {
  ...
  if (!upstreamPath) {
    return Response.json({ message: 'Unknown voice operation' }, { status: 404 });
  }
