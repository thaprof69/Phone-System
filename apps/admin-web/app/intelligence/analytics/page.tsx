...
export default async function AnalyticsPage({
  ...
  if (!response.ok) {
    return (
      ...
      >
        <LoadFailure subject="Analytics" reason={response.reason} />
      </DomainPage>
    );
  }
        ...
        <div className="range-switch" role="group" aria-label="Date range">
          {RANGES.map((range) => (
            <Link
              ...
              href={`/intelligence/analytics?days=${range}`}
      ...
      {hasEvidence ? (
        <>
          ...
          <div className="chart-columns">
            <Panel title="Why people call" eyebrow="Intent mix">
              <BarChart
                title="Calls by reason"
                description="The classified primary reason for each call."
                horizontal
                height={340}
                data={data.byIntent.slice(0, 8).map((entry) => ({
