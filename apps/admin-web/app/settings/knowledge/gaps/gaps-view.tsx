...
export async function KnowledgeGapsView({ eyebrow }: { eyebrow: 'Knowledge' | 'Intelligence' }) {
  // Same records, same view, two domains: Intelligence keeps the generic domain
  // frame (horizontal sub-nav), Knowledge renders inside the Settings vertical
  // rail — each page uses whichever frame its own pathname actually belongs to.
  const Frame = eyebrow === 'Intelligence' ? DomainPage : SettingsPage;
