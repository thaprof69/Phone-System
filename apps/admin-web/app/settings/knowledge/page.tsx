...
export default async function KnowledgeLibraryPage({
  ...
  return (
    ...
    >
      ...
      <FilterBar
        action="/settings/knowledge"
        resetHref="/settings/knowledge"
      ...
      >
        <DataTable
          ...
          rowHref={(row) => `/settings/knowledge/${row.id}`}
          sort={sort}
          buildSortHref={(key, direction) =>
            buildHref('/settings/knowledge', params, { sort: key, dir: direction, page: undefined })
          }
          empty={
            <EmptyState
        ...
        <Pagination
          ...
          total={sorted.length}
          buildHref={(next) => buildHref('/settings/knowledge', params, { page: next })}
        />
      </Panel>
    </DomainPage>
  );
