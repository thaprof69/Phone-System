...
export function MonitoringView({ summary }: { summary: MonitoringSummary }) {
  ...
  const stateColumns: Column<{ state: string; count: number }>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <Link className="row-link" href={`/administration/ai?area=execution&state=${row.state}`}>
          View runs
        </Link>
      ),
    },
    ...
    <>
      {totals.runs === 0 ? (
        <Banner tone="info" title="No runs recorded yet">
          Rates are withheld rather than shown as perfect. A failure rate with nothing behind it is
          unknown, not zero.
        </Banner>
      ) : null}
      ...
      {degradedProviders.length > 0 ? (
        ...
        >
          Degraded readings are kept rather than smoothed away, because the pattern is what explains
          a spike in fallbacks.
        </Banner>
      ) : null}
      ...
      <MetricGrid>
        ...
        <MetricCard
          label="Failure rate"
          value={totals.failureRate === null ? 'No data yet' : formatPercent(totals.failureRate)}
          detail={`${formatNumber(totals.failed)} runs produced no artefact`}
          tone={rateTone(totals.failureRate, 0.1)}
        />
        <MetricCard
          label="Fallback rate"
          value={totals.fallbackRate === null ? 'No data yet' : formatPercent(totals.fallbackRate)}
          ...
          {summary.models.map((model) => (
            <li key={model.id}>
              ...
              <StatusPill tone={model.available && !model.deprecated ? 'good' : 'danger'}>
                {model.deprecated ? 'Deprecated' : model.available ? 'Available' : 'Unavailable'}
              </StatusPill>
            </li>
          ))}
        </ul>
