...
## Architecture-Driven Full-Stack Master Production Build Prompt for Codex

---

## Executive summary and business context

Build the complete, production-grade **Quantum Parks ElevenLabs AI Receptionist Control, Knowledge, Testing, Operations, and Intelligence Platform** from the supplied corrected PRD v2.0 and Architecture Pack v2.0.

This is a one-shot engineering mandate. It is not a request for an MVP, prototype, proof of concept, demo, architecture proposal, phased roadmap, scaffold, or another implementation plan. The architecture and product decisions have already been made. Your task is to inspect the repository, load the governing artifacts, implement the full system end to end, execute it locally, test it comprehensively, inspect every critical frontend workflow, remediate defects, provide infrastructure and operational controls, and leave the repository in a condition suitable for staging validation and controlled production onboarding.

### What the product is

Quantum Parks is not building a telephone carrier, PBX, real-time media gateway, live speech-recognition engine, speech-synthesis engine, or conversational turn engine. **ElevenLabs is the managed live voice receptionist.** It receives telephone calls through the approved route, speaks and listens to callers, executes the published agent configuration, uses the assigned knowledge and approved business tools, performs configured transfers, and emits conversation transcripts and provider metadata.

**The Quantum Parks application is the product and the system of record.** It is the branded full-stack control plane through which Quantum Parks users configure, test, publish, govern, monitor, and improve the receptionist without needing the ElevenLabs console for normal operations. It is also the post-call operational and intelligence platform that turns provider transcripts into canonical call history, evidence-linked summaries, classifications, outcomes, customer timelines, tasks, callbacks, messages, quality review, trends, reports, recommendations, audit evidence, and management insight.

The platform must therefore provide two tightly integrated but clearly separated planes:

1. **ElevenLabs runtime plane** - the managed live conversational voice runtime.
2. **Quantum Parks control and intelligence plane** - the authoritative local application, business APIs, governance, knowledge, testing, release, history, analytics, and operations system.

### What users must be able to do entirely inside the Quantum Parks application

Authorized users must be able to connect and monitor the provider workspace; manage local agents and immutable agent versions; configure prompts, policy fragments, greetings, disclosures, languages, voice assignments, tools, transfer routes, workflows, dynamic variables, and post-call settings; upload and author company knowledge including FAQs, SOPs, policies, terms, schedules, prices, packages, safety procedures, and park information; review and approve knowledge; publish exact approved release manifests into ElevenLabs; discover and preview voices; govern custom voices and consent; create and run factual, behavioural, tool, transfer, multilingual, safety, injection, and regression tests; compare candidate versions; approve, publish, verify, monitor, reconcile, and roll back releases; review calls and transcripts; correct canonical records without altering raw evidence; generate Quantum Parks-owned summaries and classifications; manage callbacks, tasks, handoffs, and customer messages; search customer communication history; analyse trends and knowledge gaps; and produce operational, strategic, quality, cost, release-performance, and restricted reports.

Normal users must not need to sign in to ElevenLabs. A restricted vendor administrator may use the provider console only for activities not available through supported APIs, such as initial vendor-account bootstrap, billing, commercial contracting, account recovery, or an explicitly registered capability gap.

### Core source-of-truth model

Quantum Parks is authoritative. ElevenLabs holds only approved runtime copies.

- The local database owns every agent definition, prompt, knowledge version, voice assignment, tool contract, transfer policy, test definition, approval, release manifest, and audit record.
- Publication is a controlled compilation and synchronization operation, not direct ad hoc editing.
- Every remote provider object is mapped to a local object/version and reconciled.
- Direct changes made in the provider are detected as drift and block release until resolved.
- Provider read-back and checksum/version comparison are required before a local release is considered synchronized or active.
- Raw ElevenLabs events and provider transcripts are immutable evidence. Quantum Parks creates a separate canonical transcript with redaction, quality metadata, provenance, and versioned corrections.
- Call data may generate recommendations or knowledge-gap candidates, but it may never automatically rewrite the production agent.

### Core anti-hallucination and safety model

The platform must prevent hallucination and false completion through code-owned guardrails, not by trusting a general-purpose prompt.

- Stable company facts come only from approved, effective, scoped knowledge.
- Dynamic facts come only from authoritative typed tools and system events.
- Customer or booking details require the configured verification policy.
- The provider cannot invent tool names, URLs, arguments, SQL, code, credentials, or actions.
- Model outputs are schema-constrained, evidence-linked, policy-checked, and rejected when malformed or unsupported.
- A request is not an outcome. The system may say an action completed only after a trusted success result or system event.
- Low confidence produces a clarification, official link, human transfer, callback, task, or knowledge gap - never a guess.
- Caller speech, support content, public webpages, and uploaded documents are untrusted data and cannot override policy or authorize tools.
- Payment-card information must never be requested or intentionally retained. Sensitive cases suppress commercial behaviour and use approved routing and access controls.
- Production knowledge, prompts, tools, voices, and policies change only through human review, approvals, critical regression tests, controlled publication, remote verification, and monitored rollout.

### Full-stack result expected

Deliver a complete monorepo containing the authenticated Admin Web control plane, a separate Customer Web application for secure public flows, the backend API and ElevenLabs-facing webhooks/tools, Temporal workers, a deterministic provider simulator, authoritative PostgreSQL data model and migrations, Redis and object storage integration, provider adapters and reconciliation, full agent/knowledge/voice/test/release lifecycles, post-call ingestion and intelligence pipelines, accessible dashboards and operational workflows, reporting aggregates, OIDC/RBAC, audit, retention/deletion/legal-hold controls, observability, cost metering, CI/CD, non-root containers, Terraform for the reference AWS deployment, security and privacy documentation, runbooks, tests, release evidence, and exact external onboarding instructions.

Build every requirement in FR-01 through FR-82 and NFR-01 through NFR-18. Do not reduce scope because the task is large. Use a living execution record internally, but proceed directly through implementation as one continuous assignment. Do not answer this prompt with another plan.
...
## Governing artifacts and source-of-truth hierarchy

This build is architecture-driven. The supplied artifacts are not background reading; they are executable governance and must be reflected in code, tests, CI, documentation, and release evidence.

Locate the supplied files even if the repository uses different paths. The expected structure is:

```text
docs/product/
  Quantum_Parks_ElevenLabs_AI_Receptionist_PRD_v2.0.docx
  Quantum_Parks_ElevenLabs_One_Shot_Production_Build_Prompt_v2.0.md
docs/architecture/Quantum_Parks_ElevenLabs_Architecture_Pack_v2.0/
  ARCHITECTURE.md
  README.md
  adr/
  guardrails/
...
1. Legal safety, privacy, security, payment-data exclusion, identity-verification, sensitive-case handling, and explicit no-capacity/no-payment boundaries.
2. The corrected PRD v2.0 as the product authority.
3. The Architecture Pack v2.0, its ADRs, guardrail standards, contracts, matrices, and diagrams as the technical authority.
4. This architecture-driven master build prompt as the execution contract.
5. Healthy existing repository conventions and instructions that do not conflict with the authorities above.
6. Reasonable engineering assumptions recorded in a new ADR, compliance matrix entry, and risk/open-decision record.

The Architecture Pack supersedes earlier designs that placed a custom media gateway, custom live speech stack, or provider-neutral voice runtime at the centre of the product. Preserve earlier business requirements only where they remain compatible with the corrected ElevenLabs boundary.

Do not silently reinterpret a requirement. When a genuine contradiction remains, choose the higher authority, implement the safest buildable interpretation, document the decision in an ADR, and continue.

### Required architecture-pack reading order

Before feature work, read and index:

1. `ARCHITECTURE.md` in full.
2. All ADRs under `adr/`.
3. `guardrails/architecture-rules.yaml`.
4. Every Markdown standard under `guardrails/`.
5. Contract schemas under `contracts/`.
6. CSV matrices under `matrices/`, especially requirements traceability, component catalogue, API inventory, integration matrix, event inventory, RBAC, release gates, provider capabilities, SLOs, risks, and open decisions.
7. Editable diagrams and system boundaries under `diagrams/`.
8. The official-provider reference register, revalidating all time-sensitive claims against current official documentation before coding.

The implementation must preserve these artifacts under version control and keep them synchronized with the actual system. If generated views are created from machine-readable sources, document the generator and check drift in CI.

## Mandatory first actions

Before implementing product features:

1. Inspect the entire repository and determine whether it is greenfield or established. Read every repository instruction, README, package manifest, environment example, CI workflow, infrastructure file, schema, migration, test configuration, and existing architecture document.
2. Preserve the PRD, architecture pack, source prompt, and source manifest under `docs/product/source/` and `docs/architecture/source/` as appropriate. Generate a checksum manifest. Do not silently rewrite governing source documents.
3. Copy and tailor `guardrails/AGENTS.md.template` into the repository root as `AGENTS.md`. Preserve every architecture and security rule. Add exact local commands and repository conventions without weakening the template.
4. Create `PLANS.md` defining the durable execution-record format and create `docs/exec-plans/quantum-parks-elevenlabs-architecture-build.md`. Maintain it continuously as evidence of implementation, decisions, commands, failures, corrections, and remaining external gates. It is not a substitute for implementation and must not become another planning-only response.
5. Import `guardrails/architecture-rules.yaml` into an enforceable repository policy or architecture-fitness test. CI must fail on forbidden primary components, browser secret exposure, domain-to-provider coupling, missing tool contracts, evidence-free summaries, skipped critical tests, drifted production dependencies, or synthetic production configuration.
6. Create `docs/product/requirements-traceability.md` mapping FR-01 through FR-82 and NFR-01 through NFR-18 to code modules, API operations, domain events, database entities, UI workflows, security controls, tests, metrics, alerts, runbooks, release evidence, and external dependencies. Seed it from `matrices/requirements-architecture-traceability.csv` and maintain it as code changes.
7. Create `docs/architecture/compliance-matrix.md` and map every component, boundary, ADR, guardrail, contract, API, event, SLO, release gate, and risk to implementation evidence. Any deviation requires a new ADR and an updated compliance entry.
8. Create `docs/onboarding/external-dependencies.md` covering ElevenLabs workspace/account tier, credentials, telephony routing, provider privacy and retention choices, business APIs, approved company content, voices/consent, human queues, messaging senders/templates, OIDC, cloud accounts, DNS/certificates, native-language approval, and legal/privacy decisions.
9. Establish a clean baseline by running current formatting, linting, type checking, tests, builds, migration checks, dependency audits, secret scans, and infrastructure validation. Record pre-existing failures separately and never conceal them.
10. Verify every time-sensitive ElevenLabs endpoint, SDK, request/response field, webhook signature mechanism, account-tier feature, versioning capability, test API, knowledge operation, voice operation, telephony option, region, retention control, data-residency feature, and deprecation against current official ElevenLabs documentation. Pin verified dependencies and record substitutions in an ADR and provider capability matrix.
11. Generate or validate the repository dependency graph. Domain packages must not import ElevenLabs SDK DTOs. Browser code must not import server SDKs, database drivers, or secrets. Provider payloads must terminate at the adapter boundary.
12. Create the initial release-evidence file at `docs/qa/release-evidence.md` before implementation and append real command output, test results, screenshots, traces, fixtures, and limitations throughout the build.
...
The Architecture Pack is a build contract. Implement it as code and automated checks, not as documentation-only intent.

### Non-negotiable system boundary

- ElevenLabs is the managed live voice runtime and telephone receptionist.
- Quantum Parks is the exclusive normal-user control plane, local source of truth, knowledge-governance system, test-and-release system, canonical call-history system, and operational intelligence platform.
- The primary runtime must not contain a custom PBX, carrier, media-stream gateway, live STT/TTS orchestration layer, or custom turn engine.
- All normal operator, editor, approver, QA, analyst, and administrative workflows must be available in the Quantum Parks application. The ElevenLabs console may remain only for tightly restricted vendor-account, billing, contracting, initial credential, recovery, or currently API-inaccessible functions recorded in the provider capability register.

### Local-master / remote-runtime-copy rule

For agents, prompts, knowledge, voices, tests, tools, transfer policies, webhook settings, and release manifests:

- Quantum Parks stores the authoritative local object and immutable versions.
- ElevenLabs receives an approved runtime representation.
- Every remote object has a local-to-provider mapping, checksum/version evidence, synchronization state, and reconciliation history.
- Direct remote edits are drift, never an authoritative update.
- Provider data may not overwrite the approved local master automatically.
- Production activation requires provider read-back and proof that the remote runtime matches the approved local release manifest.

Implement synchronization states equivalent to:
...
Create automated checks that fail when:

- A custom media gateway or custom live speech runtime is added as a primary component.
- Browser bundles contain ElevenLabs service credentials, server SDKs, secret-manager clients, or database drivers.
- Domain packages import ElevenLabs DTOs or SDK types.
- Provider endpoints lack schema validation, authentication/signature checks, timeout handling, explicit result mapping, redacted logging, and contract tests.
- An agent or knowledge release bypasses server-owned tests, approvals, dependency validation, provider publication, read-back, or drift checks.
- A tool lacks a stable registry key, code-owned input/output schemas, business purpose, verification level, result union, timeout/rate policy, audit, and journey tests.
- A summary or classification lacks evidence references, model/prompt/schema versions, provenance, and correction history.
- Active production configuration references synthetic, draft, expired, inactive, sandbox, local, simulator, missing, drifted, or unapproved dependencies.
- Critical tests are skipped, pending, quarantined, or replaced by assertions without executed evidence.
- A migration is destructive without an approved expand/contract or rollback strategy.

### Runtime hallucination and false-completion guardrails

Build layered controls rather than relying on a prompt alone:
...
4. Exact approved knowledge release manifest.
5. Typed tool definitions with explicit result semantics.
6. Minimum dynamic variables.
7. Caller speech, support context, webpages, URLs, and uploaded documents labelled and delimited as untrusted data.

Stable company facts such as opening hours, ages, prices, packages, rules, policies, and processes may be stated as certain only from active, approved, effective knowledge. Dynamic facts such as customer identity, bookings, availability, transfer completion, message delivery, or action completion require an authoritative tool/system result.

No agent response, summary, task, report, or UI may convert a request or attempt into a completed action. Completed-action language is allowed only when the trusted result is `SUCCESS`. Keep separate fields for requested, attempted, queued, succeeded, failed, and unconfirmed.

All model-produced intent, entity, summary, classification, recommendation, and next-step output must use code-owned JSON schemas. Reject malformed or unknown privileged fields. Consequential claims require transcript-segment or trusted-event evidence references. Apply deterministic policy after model output and before persistence, external action, customer messaging, or release.

Caller speech, retrieved knowledge, support tickets, public webpages, uploaded documents, and provider payloads are untrusted data. They may not create tools, change policy hierarchy, request secrets, authorize actions, or supply arbitrary URLs, SQL, code, or provider credentials.

Calls may create knowledge-gap candidates and improvement recommendations. They must never automatically modify production prompts, knowledge, tools, transfer rules, policies, or agent configuration. The only route into production is human-authored change, review, approval, regression testing, publication, remote verification, and monitored release.

### Evidence discipline for Codex

Do not invent provider endpoints, capabilities, entitlements, package versions, request fields, model names, regions, quotas, pricing, test results, screenshots, load results, security results, or claims of production readiness. When a live capability cannot be verified or used, implement the adapter, simulator, contract tests, readiness gate, runbook, and exact unblock instructions, then mark it `EXTERNALLY_BLOCKED`.
...
## Architecture-pack implementation evidence

Before the general Definition of Done can pass, demonstrate that the architecture pack has been implemented rather than merely copied:

- Every ADR has an implementation reference, test reference, or an explicitly documented external dependency.
- `architecture-rules.yaml` is enforced by CI or equivalent fitness tests.
- Contract schemas under `contracts/` are implemented or superseded by compatible versioned schemas with drift tests.
- Every row in the component, API, integration, event, RBAC, data-classification, SLO, release-gate, provider-capability, risk, and requirements matrices maps to actual code and evidence.
- All fourteen architecture diagrams are updated when implementation materially differs and match the deployed system.
- The provider simulator covers successful operations, capability denial, rate limits, timeouts, invalid payloads, duplicate/out-of-order webhooks, signature failure, drift, missing objects, unavailable voices, failed publication, and conversation reconciliation.
- Agent releases and application releases are independently versioned, tested, approved, published, observed, and rollback-capable.
- A full local-master-to-ElevenLabs publish/read-back/reconciliation journey passes against the simulator and, where credentials exist, a non-production workspace.
- A complete post-call journey passes from signed webhook through raw evidence, normalized provider transcript, canonical transcript, evidence-linked summary, classification, outcome, customer timeline, message/task, aggregates, dashboard, and report.
- A knowledge change passes authoring/upload, malware and file checks, extraction, injection scanning, duplicate/conflict/staleness checks, approval, immutable release manifest, provider publication, dependency verification, regression test, activation, and rollback.
- The Test Studio demonstrates deterministic and model-assisted evaluations, repeated critical runs, human-review requirements, and release blocking.
- Drift in agent configuration, knowledge dependencies, voice availability, tests, webhook subscriptions, or conversation completeness is detected, alerted, and reconciled.
- Sensitive-case, payment, verification, no-capacity, prompt-injection, and no-false-completion scenarios pass the architecture pack's critical gates.
...
Begin now. Read the governing artifacts, establish the repository controls, then implement the entire production system. Do not pause for routine confirmation and do not return another planning-only response. Continue until the full Definition of Done is satisfied or the only remaining gaps are genuine external onboarding dependencies recorded with exact unblock steps and an honest `EXTERNALLY_BLOCKED` readiness state.
