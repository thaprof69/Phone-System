...
| Live ElevenLabs voice session      | `voice_sessions` table, `PlatformService.startVoiceSession/attachVoiceSessionConversation/endVoiceSession`, real `getAgent`/`getSignedConversationUrl` adapter calls, `@elevenlabs/client` in Simulation Lab | Adapter unit tests, admin browser tests, real simulator round trip (signed URL, blocked/agent-unavailable refusals) |
